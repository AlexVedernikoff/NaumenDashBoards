# Встроенное приложение "Календарь"
...
### Настройка
Используется кастомная тема для компонетна календаря KendoReact Scheduler.
В случае необходимости изменить внешний вид внутренней реализации календаря, следует редактировать файл calendar/src/styles/customTheme.css. В случае необходимости полной замены темы, следует использовать Theme Builder: https://themebuilder.telerik.com/kendo-react-ui
...
Система сборки (`webpack@5`) и утилита преобразования `ES6` (`babel`) в `JavaScript` не подразумевают работу результата в `Internet Explorer 8`.
