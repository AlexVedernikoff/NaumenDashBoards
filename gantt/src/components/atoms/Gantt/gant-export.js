...
(function () {
	...
	function add_export_methods (gantt) {
					...
					if (settings.parsed) {
						...
						for (const i in settings.parsed.dates) {
							if (Array.isArray(settings.parsed.dates[i])) {
								time.dates[i] = getMinutesWorktimeSettings(
									settings.parsed.dates[i]
								);
							} else {
		...
		gantt._send_to_export = function (data, type) {
			const convert = this.date.date_to_str(
			...
			form.firstElementChild.action = data.server || apiUrl;
			form.firstElementChild.childNodes[0].value = JSON.stringify(data);
			form.firstElementChild.childNodes[1].value = type;
		...
		function copy_object_table (obj) {
			...
			const getDayIndex = gantt._day_index_by_date
		...
		function copy_object_colors (obj) {
			...
			if (node && node.firstElementChild) {
				let color = get_color(
					gantt._no_progress_colors ? node : node.firstElementChild,
					'backgroundColor'
				);
		...
		function get_raw () {
			...
			if (gantt._scale_helpers) {
				...
				const autosize_min_width = gantt._get_resize_options().x
		...
		gantt._serialize_scales = function (config) {
			...
			for (var i = 0; i < raw.length; i++) {
				...
				for (let j = 0; j < step.trace_x.length; j++) {
					...
					const scale_cell = { end: end, start: start, text: template(date) };
			...
			return { data: scales, height: scales.length, width: max };
		};
...
(function () {
	...
	function exportConfig (data, config) {
		...
		if (p_dates.start_date && p_dates.end_date) {
			const formatDate
				= gantt.templates.format_date || gantt.templates.xml_format;
	...
	function add_export_methods (gantt) {
		...
		gantt.exportToMSProject = function (config) {
			...
			this.templates.xml_format = this.date.date_to_str(
				exportServiceDateFormat
			);
			this.templates.format_date = this.date.date_to_str(
				exportServiceDateFormat
			);
			const data = this._serialize_all();
