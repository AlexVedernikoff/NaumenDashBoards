...
const multipleMarker = (color) => (`<svg width="26" height="32" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M8 8c-4.42 0-8 3.76-8 8.4C0 22.7 8 32 8 32s8-9.3 8-15.6C16 11.76 12.42 8 8 8z" fill="${color}"/><circle cx="8" cy="16" r="2" fill="#fff"/><circle cx="18" cy="8" r="7.5" fill="#fff" stroke="${color}"/></svg>`);
const multipleMarkerHover = () => (`<svg width="29" height="37" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M10 6C4.47 6 0 10.85 0 16.85 0 24.99 10 37 10 37s10-12.01 10-20.15C20 10.85 15.53 6 10 6z" fill="#EB5757"/><circle cx="10" cy="15.25" r="2.5" fill="#fff"/><circle cx="21" cy="8" r="7.5" fill="#fff" stroke="#EB5757"/></svg>`);
...
export const divIconMarker = (iconName: IconName, color: string, count: number) => {
	...
	const divIconData = {
		html: `<div class="${iconName}"><img src=${url} /><div class='${iconName}Count countMarker'>${count.toString()}</div></div>`
	};
