...
export class RangeField extends PureComponent<Props> {
	...
	handleChangePercentRange = (event: OnChangeEvent<string>) => {
		...
		if (/^(\d{1,3})?$/.test(value) && (name !== DIAGRAM_FIELDS.to || Number(value) <= 100)) {
	...
	renderRemoveIcon = () => {
		...
		return (
			<div className={iconCN} onClick={this.handleRemove} title={t('BorderRangeField::Delete')}>
