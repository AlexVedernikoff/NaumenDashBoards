...
import {getYAxisDomainNiceScale, getYAxisNumber, normalizeSeries} from './columns.helpers';
...
const getOptions = (
	...
	if (axisWidget) {
		...
		if (buildDataSet) {
			...
			const subTotalGetter = showSubTotal
				? makeSubTotalGetter(axisWidget, seriesData, usesPercent)
				: null;
			const legend = getLegendOptions(container, axisWidget.legend);
			const yAxis = getYAxisNumber(axisWidget, rawData, formatters.indicator, yAxisName);
			const xAxis = getXAxisCategory(
				axisWidget,
				container,
				labels.map(formatters.parameter),
				yAxis,
				xAxisName
			);
			...
			yAxis.domain = getYAxisDomainNiceScale(axisWidget, container, xAxis, yAxis, legend, false);
