...
/**
 * Графики с включенной опцией отображения значений выдают ошибку при конвертации в png. В чем проблема пока не известно.
 * Проблема открыта тут https://github.com/canvg/canvg/issues/816
 * Для корректной работы добавил нужное условие с пометкой TODO
 */
	...
	function build(opts) {
	  ...
	  svg.Element.filter = function (node) {
	    ...
	    this.apply = function (ctx, element) {
	      ...
	      // TODO необходимое условие для корректной работы
	      if (bb) {
					var x = Math.floor(bb.x1);
					var y = Math.floor(bb.y1);
					var width = Math.floor(bb.width());
