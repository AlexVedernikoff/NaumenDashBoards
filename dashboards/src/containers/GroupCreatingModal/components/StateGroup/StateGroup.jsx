...
import type {
	...
	OperandType,
	RefOrCondition, SelectOperand as SelectOperandType, SimpleOperand as SimpleOperandType
...
export class StateGroup extends Component<Props, State> {
	state = {
		updateDate: new Date()
	};

	componentDidUpdate (prevProps: Props) {
		const {attribute, states: nextStates} = this.props;
		const {states: prevStates} = prevProps;
		const {metaClassFqn} = attribute;

		if (prevStates[metaClassFqn] !== nextStates[metaClassFqn]) {
	...
	getCustomGroups = (): Array<CustomGroup> => this.props.customGroups.filter(({type}) => type === ATTRIBUTE_TYPES.state);
	...
	getSelectData = () => {
		const {attribute, states} = this.props;
		return states[attribute.metaClassFqn];
	};
