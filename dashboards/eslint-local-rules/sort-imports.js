...
const fixSortImports = (sourceCode, imports = []) => fixer => {
	...
	const newImportsTexts = newImports.map(imp => sourceCode.getText(imp)).join('\n');

	return [
		...imports.slice(1).map(imp => fixer.remove(imp)),
		fixer.replaceText(imports[0], newImportsTexts)
	];
};

const fixSortSpecifiers = (sourceCode, imp) => fixer => {
	const otherSpecifiers = imp.specifiers.filter(specifier => specifier.type !== 'ImportDefaultSpecifier');
...
const create = context => ({
	Program: function (program) {
		...
		try {
			...
			imports.forEach((imp, index, imports) => {
				if (index > 0 && !hasSpecifiers(imp) && hasSpecifiers(imports[index - 1])) {
					throw createError(imp, 'Импорты модулей для использования их побочных эффектов должны быть в самом начале', fixSortImports(sourceCode, imports));
				}
			});

			// Проверка на алфавитный порядок импортов без спецификаторов
			imports.filter(imp => !hasSpecifiers(imp)).forEach((imp, index, imports) => {
				if (index < imp.length - 1 && hasAlphabetOrderError(imp.source.value, imports[index + 1].source.value)) {
					throw createError(imp, 'Импорты модулей для использования их побочных эффектов должны быть отсортированы в алфавитном порядке', fixSortImports(sourceCode, imports));
				}
			});

			// Проверка импортов с указанными спецификаторами
			imports.filter(hasSpecifiers).forEach((imp, index, imports) => {
				const {specifiers} = imp;
				...
				specifiers.filter(s => s.type !== 'ImportDefaultSpecifier').forEach((specifier, index, specifiers) => {
					if (index < specifiers.length - 1 && hasAlphabetOrderError(specifier.imported.name, specifiers[index + 1].imported.name)) {
						throw createError(specifier, 'Импортируемых значений внутри импорта должен быть в алфавитном порядке ', fixSortSpecifiers(sourceCode, imp));
					}
				});
				...
				if (index < imports.length - 1 && hasAlphabetOrderError(getSortName(specifiers[0]), getSortName(imports[index + 1].specifiers[0]))) {
					throw createError(imp, 'Блок импортов должен быть отсортирован в алфавитном порядке', fixSortImports(sourceCode, imports));
				}
			});
		} catch (error) {
