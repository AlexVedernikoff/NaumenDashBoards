...
@Field @Lazy @Delegate DashboardDataSet dashboardDataSet = new DashboardDataSetImpl(
    binding, new DashboardDataSetService(
    api.utils,
    api.metainfo,
    api.listdata,
    api.types,
...
class DashboardDataSetImpl extends BaseController implements DashboardDataSet
{
    ...
    String getDataForCompositeDiagram(Map<String, Object> requestContent, IUUIDIdentifiable user)
    {
        GetGataForCompositeDiagramRequest request = new ObjectMapper()
            .convertValue(requestContent, GetGataForCompositeDiagramRequest)
        ...
        return user
            ? api.auth.callAs(user) {
            service.buildDiagram(
                request.dashboardKey,
                request.widgetKey,
                request.cardObjectUuid,
                widgetFilters,
                request.offsetUTCMinutes,
                user,
                requestContent
    ...
    String getDataForTableDiagram(Map<String, Object> requestContent, IUUIDIdentifiable user)
    {
        GetDataForTableDiagramRequest request = new ObjectMapper()
            .convertValue(requestContent, GetDataForTableDiagramRequest)
        Collection<WidgetFilterResponse> widgetFilters
        if (request.widgetFilters)
        {
            widgetFilters = WidgetFilterResponse.getWidgetFiltersCollection(request.widgetFilters)
...
class DashboardDataSetService
{
     ...
     * @return тело запроса
     */
    Widget getWidgetSettingsByDashboardSettingsAndWidgetKey(DashboardSettingsClass dbSettings,
                                                            String widgetKey,
                                                            String currentUserLocale)
    {
        def widget = dbSettings.widgets.find {
            it.id == widgetKey
        }
        if (widget)
        {
            widget.data = widget.data.collect { widgetData ->
                ...
                widgetData.breakdown?.findResults {
                    it.group = Group.mappingGroup(
                        it.group,
                        dbSettings?.customGroups,
                        !widgetData?.breakdown?.any()
                    )
                    return it
                }
                     ...
                     Map<String, Object> requestContent)
    {
        ...
        DashboardSettingsClass dbSettings =
            dashboardSettingsService.getDashboardSetting(dashboardKey, true)
        def widgetSettings = getWidgetSettingsByDashboardSettingsAndWidgetKey(
            dbSettings,
            widgetKey,
            currentUserLocale
        )
        ...
        Boolean isSourceForEachRow =
            widgetSettings.data.sourceRowName.findAll() && diagramType == DiagramType.TABLE
        ...
        if (diagramType in [DiagramType.TABLE, DiagramType.PIVOT_TABLE])
        {
            ...
            Boolean computationInTableRequest = widgetSettings?.data?.any {
                it.indicators?.any {
                    it?.attribute?.any {
            ...
            {
                tableSorting?.accessor =
                    widgetSettings.data.find().parameters.find().attribute.title
                reverseRowCount = tableSorting?.type == SortingType.DESC
            }
            request = mappingDiagramRequest(
                widgetSettings, subjectUUID, diagramType,
            ...
            if (tableSorting?.accessor)
            {
                ...
                def attrValue = attributes.find {
                    it?.name?.trim() == tableSorting.accessor.trim()
                }
                sortingValueIsComputationAttribute = attrValue?.attribute instanceof ComputedAttr
            }

            Boolean isCodeState = widgetSettings.data?.parameters?.attribute?.code?.any {
                it.any {
                    it == 'state'
                }
            }

            Boolean noPaginationInSQL = requestHasBreakdown ||
                                        innerCustomGroupNames ||
                                        sortingValueIsComputationAttribute ||
                                        tableTop || isCodeState
            res = getDiagramData(
                request,
                user,
                diagramType,
            ...
            if (!(requestHasBreakdown && computationInTableRequest))
            {
                ...
                if (fullRes?.find())
                {
                    ...
                    else
                    {
                        ...
                        Integer finalIndex =
                            aggregationsSize > tempTransponseRes.size() ? tempTransponseRes.size() :
                                aggregationsSize
        ...
        else
        {
            ...
            countTotals =
                getTotalAmount(request, res, diagramType, templateUUID, user, widgetSettings)
            if (diagramType == DiagramType.SPEEDOMETER)
            {
                minValue = getValueForBorder(
                    widgetSettings,
                    subjectUUID,
                    diagramType,
                    widgetFilters,
        ...
        switch (diagramType)
        {
            ...
            case DiagramType.RoundTypes:
                ...
                Integer percentCntAggregationIndex =
                    percentCntAggregationIndexes ? percentCntAggregationIndexes.first() : null
                Boolean checkTypeMetaClass = widgetSettings?.data?.each { value ->
                    value?.indicators?.any {
                        it?.aggregation == "PERCENT_CNT"
                    }
                }
                return mappingRoundDiagram(
                    res,
                    countTotals,
                    percentCntAggregationIndex != null || checkTypeMetaClass
                )
            ...
            case [TABLE, PIVOT_TABLE]:
                ...
                return mappingTableDiagram(
                    res,
                    totalColumn as boolean,
                    showRowNum as boolean,
                    rowCount,
                    tableTop,
                    paginationSettings,
                    tableSorting,
                    ...
                    countTotals,
                    tableTotals,
                    widgetSettings.data.sourceRowName.findAll(),
                    diagramType,
                    user
                )
            case COMBO:
                ...
                List<Boolean> customsInBreakdown =
                    isCustomGroupFromBreakdown(widgetSettings, diagramType)
                customsInBreakdown = sortListsForCombo(customsInBreakdown, sortingDataIndex)
                return mappingComboDiagram(
                    res, additionals, groupFormat, format,
            ...
            default:
                ...
                return
                utils.throwReadableException("${ message }#${ NOT_SUPPORTED_DIAGRAM_TYPE_ERROR }")
        }
    }
     ...
     */
    private List applyIndicatorsFiltration(RequestData requestData,
                                           List res,
                                           IUUIDIdentifiable user)
    {
        ...
        aggregationsWithFiltration.each { aggregation ->
            ...
            Closure formatGroup = this.&formatGroupSet
                .rcurry(requestDataCopy, listIdsOfNormalAggregations, DiagramType.PIVOT_TABLE)
        ...
        aggregationFiltrationResult.eachWithIndex { aggregation, filtrationResult, index ->
            Integer aggregationIndex = allAggregations.findIndexOf {
                it == aggregation
            }
                    ...
                    if (value == 0)
                    {
                        ...
                        for (int i = 1; i < groupsCount + 1; i++)
                        {
                            filtrationResultItemMatch = filtrationResultItemMatch &&
                                                        ...
                                                        filtrationResultItem[
                                                            filtrationResultItem.size() - i
                                                        ]
                        }
                                          ...
                                          Widget widgetSettings)
    {
        ...
        aggregationsWithBreakdown.each { aggregation ->
            ...
            replaceResultAttributeMetaClass(
                result,
                new DiagramRequest(
                    data: [(requestDataCopy.source.dataKey): requestDataCopy]
                )
            )
        ...
        aggregationBreakdownResult.eachWithIndex { aggregation, breakdownResult, index ->
            ...
            res[0].eachWithIndex { resultItem, resIndex ->
                ...
                breakdownResult.each { breakdownResultItem ->
                    ...
                    for (int i = breakdownResultItem.size() - 1;
                         i > resultItem.size() - groupsCount - (aggregationsCount - 1); i--)
                    {
                        breakdownResultItemMatch =
                            breakdownResultItemMatch &&
                            resultItem[i - 1 + (aggregationsCount - 1)] ==
                            breakdownResultItem[i - 1]
                    }
                                     ...
                                     IUUIDIdentifiable user)
    {
        ...
        if (!(field.isNumber))
        {
            ...
            widgetSettings?.data?.find {
                !it.sourceForCompute
            }?.indicators = [fieldAggregation]
    ...
    {
        Integer total = 0
        if (widgetSettings.type in [*DiagramType.CountableTypes, DiagramType.TABLE] &&
            widgetSettings.showTotalAmount)
        {
            if (isSourceForEachRow)
            {
            ...
            else
            {
                ...
                Boolean tableWithNA = diagramType == DiagramType.TABLE &&
                                      request?.data?.any { key, value ->
                                          value.aggregations.type.any {
                                              it == Aggregation.NOT_APPLICABLE
                                          }
                                      }
                if (tableWithNA)
                {
                    ...
                    total = res?.find()?.transpose()?.get(NAIndex)?.count {
                        it != ''
                    }
                }
                else
                {
                    ...
                    {
                        total = res.grep()*.transpose()*.get(0).flatten().sum {
                            it as Integer
                        }
                    }
                    else
                    {
                        total = res?.find()?.transpose()?.find()?.sum {
                            it as Integer
                        }
                    }
                }
    ...
    {
        Boolean noCalculation = request.requisite.every {
            it.nodes.every {
                it.type == 'DEFAULT'
            }
        }
     ...
     * @return обновленный запрос на построение
     */
    private DiagramRequest updateAggregationsInTableDiagram(DiagramRequest request,
                                                            Boolean withCalculation)
    {
        ...
        if (tempData.value.aggregations.any {
            it.type != Aggregation.NOT_APPLICABLE
        })
        {
     ...
     * @return обновленный запрос на построение
     */
    private DiagramRequest updateAggregationsInBaseDiagramWithCalculation(DiagramRequest request,
                                                                          DiagramType diagramType)
    {
        if (diagramType == DiagramType.COMBO)
        {
            def requisites = request.requisite
            def normalRequisites = requisites.findAll {
                it.nodes.every {
                    it.type == 'DEFAULT'
                }
            }
            ...
            compRequisites.each { requisite ->
                requisite.nodes.each { node ->
                    ...
                    dataKeys -= dataKeys.findAll {
                        it in totalData.keySet()
                    }

                    def additionalData = dataKeys.collect { key ->
                        request.data.find {
                            it.key == key
                        }
                    }
     ...
     */
    Integer countDataForManyCustomGroupsInParameters(def requestContent,
                                                     Boolean inFirstSource = true)
    {
     ...
     */
    private Integer getRealFilterListSizeForTable(Integer filterListSize,
                                                  def requestContent,
                                                  String templateUUID)
    {
        Integer countOfCustomsInFirstSource =
            countDataForManyCustomGroupsInParameters(requestContent)
        Integer countOfCustomsInFullRequest =
            countDataForManyCustomGroupsInParameters(requestContent, false)
        Boolean requestHasDynamicAndCustom = (countOfCustomsInFirstSource > 0 ||
                                              countOfCustomsInFullRequest > 0) && templateUUID

        if (countOfCustomsInFirstSource > 1 ||
            countOfCustomsInFullRequest > 1 || requestHasDynamicAndCustom)
        {
            filterListSize = 2
        }
    ...
    private String getTemplateUUID(Widget widgetSettings)
    {
        ...
        def attrCode = places.findResult { place ->
            return widgetSettings?.data?.findResult { v ->
                return v[place]?.attribute?.code?.find {
     ...
     */
    Map<String, Map> updateIntermediateDataToOneSource(Map<String, Map> intermediateData,
                                                       Boolean computationInRequest,
                                                       Integer countOfCustomsInFirstSource)
    {
        //если агрегаций нет, то источник нужен только для вычисления
        List<RequestData> requestData = intermediateData.findResults { key, value
            ->
            value?.requestData?.aggregations ? value?.requestData : null
        }
        Source mainSource = intermediateData.findResult { key, value -> value.requestData ?: null
        }.source
        ...
        Requisite originalRequisite = intermediateData.findResult { key, value ->
            if (computationInRequest && value?.requisite?.nodes?.findAll {
                it.type == 'COMPUTATION'
            })
            {
        ...
        if (cnt > 1)
        {
            ...
            mainSource.filterList =
                breakdownFilterList ? [parameterFilters] + [breakdownFilterList] :
                    [parameterFilters]
        }
        else
        {
            mainSource.filterList = filterList.findAll {
                it?.filters
            }
        }
        ...
        List fullGroups = requestData?.collectMany {
            ...
            if (it?.source?.classFqn != mainSource.classFqn)
            {
                ...
                return groups?.collect { group ->
                    def highLevelAttr = new Attribute(
                        code: it?.source?.classFqn, sourceCode: mainSource.classFqn,
                        type: 'object',
        ...
        List fullAggregations = requestData?.aggregations?.collectMany {
            it?.any {
                it?.attribute instanceof ComputedAttr
            } ? it : it?.collect { aggr -> return prepareAggregation(aggr, mainSource.classFqn)
            }
        }

        def breakdowns = fullGroups.findAll {
            it.title == 'breakdown'
        }
        fullGroups = fullGroups - breakdowns
        fullGroups = fullGroups + breakdowns

        RequestData totalRequestData = new RequestData(
            source: mainSource,
            aggregations: fullAggregations,
            groups: fullGroups,
            filters: null
        )
        return [(totalKey): [requestData        : totalRequestData, computeData:
            computationData, customGroup        :
    ...
    Map<String, Map> updateIntermediateData(Map<String, Map> intermediateData)
    {
        ...
        def computeAggregations = defaultRequestData.aggregations.findAll {
        ...
        if (defaultRequestData.aggregations.any())
        {
            ...
            dataMap = [(keyForData): [requestData                        :
                                          defaultRequestData, computeData: null, customGroup:
        ...
        Map dataMaps = computeAggregations.withIndex().collect { aggregation, i ->
            ...
            return [(dataKey): [requestData        : tempRequestData, computeData:
                computationData[i], customGroup    :
                ...
                Map dataMaps = computeAggregations.withIndex().collect { aggregation, i ->
                    ...
                    newComputationData = newComputationData.collectEntries {
                        ...
                        if (usedComputeDataKeys.contains(it.key))
                        {
                            ...
                            compositeRequisiteNode.formula =
                                compositeRequisiteNode.formula.replaceAll(it.key, key)
                        }
                        return [key, it.value]
                    }
     ...
     */
    AggregationParameter prepareAggregation(AggregationParameter aggregationParameter,
                                            String classFqn)
    {
     ...
     */
    DiagramRequest mappingDiagramRequest(def widgetSettings,
                                         String subjectUUID,
                                         DiagramType diagramType,
                                         Collection<WidgetFilterResponse> widgetFilters,
                                         Integer offsetUTCMinutes = 0,
                                         Boolean showTableNulls = false,
                                         Boolean showTableBlanks = false,
                                         Boolean computationInTableRequest = false,
                                         Integer tableTop = 0,
                                         Sorting tableSorting = null)
    {
        ...
        Boolean hasTableNotOnlyBaseSources = (tempWidgetSettings?.data*.source.value.value as Set)
                                                 .size() > 1
        Boolean isDiagramTypeNotCount = !(
            diagramType in [DiagramType.CountTypes, DiagramType.RoundTypes])
        ...
        uglyRequestData.each { data
            ->
            def descriptor =
                DashboardMarshallerClass
                    .substitutionCardObject(data.source.descriptor as String, subjectUUID)
            ...
            List<AggregationParameter> aggregationParameters = !(sourceForCompute) ?
                indicators.collect { indicator ->
                    def sortingType
                    if (isDiagramTypeTable)
                    {
                        sortingType =
                            tableSorting?.accessor?.trim() == indicator.attribute.title.trim() ?
                                tableSorting?.type : null
                    }
                    else
                    {
                        sortingType =
                            sorting && sorting?.value == SortingValue.INDICATOR ? sorting?.type :
                                null
                    }
            ...
            aggregationParameters.each { aggregationParameter ->
                dynamicInAggregate =
                    aggregationParameter?.attribute?.code?.contains(AttributeType.TOTAL_VALUE_TYPE)
            ...
            if (isDiagramTypeNotCount)
            {
                parameters.each {
                    ...
                    {
                        sortingType =
                            sorting && sorting.value == SortingValue.PARAMETER ? sorting.type : null
                    }
                    ...
                    {
                        def newParameterFilterList = getFilterList(
                            it,
                            subjectUUID,
                            'parameter',
                            source,
                        ...
                        if (parameterFilters)
                        {
                            ...
                            Collection<Collection<FilterParameter>> parameterListFilters =
                                parameterFilters.find().filters
                            parameterListFilters =
                                parameterListFilters.collectMany { parameterFilter ->
                                    newParameterFilters.collect {
                                        //если группировка добавляется не впервые, добавляем её другим форматом
                                        (groupParameterId > 1) ? [*parameterFilter, it] :
                                            [parameterFilter, it]
                                    }
                                }
                            def filterList = new FilterList(
                                place: 'parameter',
            ...
            groupParameters.each { groupParameter ->
                dynamicInParameter =
                    groupParameter?.attribute?.code?.contains(AttributeType.TOTAL_VALUE_TYPE)
            ...
            def breakdownMap = [:]
            breakdownMap = isDiagramTypeTable && !hasTableNotOnlyBaseSources && !mayBeBreakdown &&
                           computationInTableRequest && commonBreakdown
                ...
                if (mayBeBreakdown instanceof Collection && mayBeBreakdown?.any())
                {
                    ...
                    if (groupTypes.size() == 1)
                    {
                        ...
                        breakdownMap = mayBeBreakdown
                            ...
                            .collectEntries { el ->
                                dynamicInBreakdown =
                                    el?.attribute?.code?.contains(AttributeType.TOTAL_VALUE_TYPE)
                                if (dynamicInBreakdown)
                                {
                                    dynamicGroup = mappingDynamicAttributeCustomGroup(el.attribute)
                                }
                                [(el.dataKey): buildSystemGroup(
                                    el.group,
                                    el.attribute,
                                    diagramType == DiagramType.COMBO ? 'usual_breakdown' :
                                        'breakdown'
                                )]
                            }
                        commonBreakdown =
                            isDiagramTypeTable && !hasTableNotOnlyBaseSources ? breakdownMap : [:]
                    }
                    else
                    {
                        String message = messageProvider.getMessage(
                        ...
                        return
                        utils
                            .throwReadableException("${ message }#${ GROUP_TYPES_DONT_MATCH_ERROR }")
                    }
                }
            }
            ...
            def comp = !(sourceForCompute) ? indicators?.findResults { indicator ->
                if (indicator?.attribute instanceof ComputedAttr)
                {
                    ...
                    def computeData = compData.collectEntries { k, v ->
                        String dataKey =
                            isDiagramTypeTable ? "сompute-data_${ compIndicatorId }" : v.dataKey
            ...
            def breakdownAttribute = mayBeBreakdown?.find()
            //м.б. нужно будет убрать find() и пойти в цикле
            ...
            {
                parameterFilters << getFilterList(
                    dynamicGroup,
                    subjectUUID,
                    'parameter',
                    source,
            ...
            else
            {
                ...
                Boolean showNulls =
                    isDiagramTypeTable ? showTableNulls : data.showEmptyData as Boolean
                Boolean showBlank =
                    isDiagramTypeTable ? showTableBlanks : data.showBlankData as Boolean
                Integer top = isDiagramTypeTable ? tableTop :
                    data?.top?.show ? data.top?.count as Integer : null
        ...
        Boolean manySources = isDiagramTypeTable &&
                              tempWidgetSettings?.data*.sourceForCompute?.count {
                                  !it
                              } > 1
        ...
        else if (!isSourceForEachRow &&
                 diagramType !=
                 DiagramType.PIVOT_TABLE &&
                 manySources || (hasTableNotOnlyBaseSources && computationInTableRequest))
        {
            Integer countOfCustomsInFirstSource =
                countDataForManyCustomGroupsInParameters(tempWidgetSettings)
            intermediateData = updateIntermediateDataToOneSource(
                intermediateData,
                computationInTableRequest,
                countOfCustomsInFirstSource
            )
        }
        if (computationInTableRequest)
        {
            ...
            DiagramRequest request =
                buildDiagramRequest(intermediateData, subjectUUID, diagramType, widgetSettings)

            List<RequestData> requestData = prevData.findResults { key, value
                ->
                value?.requestData?.aggregations ? value?.requestData : null
            }
            ...
            computeData?.each {
                ...
                def currentSource = request?.data?.getAt(it?.currentKey)?.source
                //здесь может быть как ссылочный атрибут,
                //так и полноценный источник - важен его код(classFqn)
                if (metainfo
                    .checkAttributeExisting(currentSource.classFqn, prevDataSource.classFqn))
                {
     ...
     */
    private Source getCorrectSource(String dataKey,
                                    ...
                                    Collection<WidgetFilterResponse> widgetFilters)
    {
        ...
        {
            WidgetFilterResponse userFilter = widgetFilters.find {
                it.dataKey == dataKey
            }
    ...
    {
        List baseDescriptorAttributes =
            baseDescriptor ? getAttributesFqnFromDescriptor(baseDescriptor) : []
        List userDescriptorAttributes =
            userDescriptor ? getAttributesFqnFromDescriptor(userDescriptor) : []
        Boolean descriptorsHaveSameAttrs = baseDescriptorAttributes.any {
            it in userDescriptorAttributes
        }
        ...
        {
            descriptorMap = updateDescriptorWithTheSameAttrs(
                descriptorMap,
                userDescriptorMap,
                baseDescriptorAttributes,
                userDescriptorAttributes
            )
        }
        else
        {
     ...
     */
    private Map updateDescriptorWithTheSameAttrs(Map descriptorMap,
                                                 Map userDescriptorMap,
                                                 List baseDescriptorAttributes,
                                                 List userDescriptorAttributes)
    {
        ...
        if (baseDescriptorAttributes -
            userDescriptorAttributes &&
            baseDescriptorAttributes.containsAll(userDescriptorAttributes))
        {
            def filters = descriptorMap.filters
            def userFilters = userDescriptorMap.filters
            descriptorMap.filters =
                putUserFiltersIntoBase(userFilters, filters, userDescriptorAttributes)
        }
        ...
        else if ((userDescriptorAttributes - baseDescriptorAttributes) && (
            baseDescriptorAttributes - userDescriptorAttributes))
        {
            ...
            descriptorMap.filters =
                putUserFiltersIntoBase(userFilters, filters, userDescriptorAttributes) + userFilters
            //убираем повторы
     ...
     */
    List<List> putUserFiltersIntoBase(List<List> userFilters,
                                      List<List> filters,
                                      List userDescriptorAttributes)
    {
     ...
     */
    private GroupParameter buildSystemGroup(def groupType,
                                            Attribute attr,
    ...
    private GroupType getDTIntervalGroupType(String groupType)
    {
        ...
        switch (groupType.toLowerCase())
        {
            ...
            default:
                ...
                utils
                    .throwReadableException("$message#${ NOT_SUPPORTED_DTINTERVAL_GROUP_TYPE_ERROR }")
        }
    }
    ...
    private DiagramRequest buildDiagramRequest(Map<String, Map> intermediateData,
                                               String subjectUUID,
                                               DiagramType diagramType = DiagramType.COMBO,
                                               Object widgetSettings)
    {
        ...
        resultRequestData =
            updateDiagramRequestDataForPercentCalculation(requisite, resultRequestData)
     ...
     */
    private Map<String, Map> mergeSourceDataForPivotTable(Map<String, Map> intermediateData,
                                                          Object widgetSettings)
    {
        String firstDataKey
                                                    ...
                                                    Map map)
    {
        if (map.computeData instanceof Collection)
        {
            ...
            return variableNames.collectEntries { variableName ->
                ...
                if (attributeType in
                    AttributeType.LINK_TYPES && attributeType != AttributeType.CATALOG_ITEM_TYPE)
                {
                ...
                newRequestData.groups = (newRequestData.groups || group) ?
                    (newRequestData.groups + group).grep() :
                    null
        ...
        else
        {
            ...
            return variableNames.collectEntries { variableName ->
                ...
                // предполагаем что количество агрегаций будет не больше одной
                newRequestData.groups = (newRequestData.groups || group)
                    ? (newRequestData.groups.findAll {
                    it?.title != 'usual_breakdown'
                } + group).grep()
                    : null
                // группировку нужно будет добавить к существующим
                newRequestData.groups = newRequestData.groups as Set
                                                                                          ...
                                                                                          Boolean fromDD = false)
    {
        switch (type)
        {
            ...
            case [AttributeType.BO_LINKS_TYPE, AttributeType.BACK_BO_LINKS_TYPE,
            ...
            default:
                ...
                utils
                    .throwReadableException("$message#${ NOT_SUPPORTED_ATTRIBUTE_TYPE_FOR_CUSTOM_GROUP_ERROR }")
        }
    }
     ...
     */
    private List<List<FilterParameter>> getDynamicFilter(List<List> data,
                                                         Attribute attribute,
                                                         String title,
                                                         String id)
    {
        ...
        def possibleFilter = mappingFilter(data) { condition ->
            ...
            switch (Condition.getByTitle(conditionType.toLowerCase()))
            {
                ...
                case Condition.CONTAINS:
                    ...
                    {
                        return
                        utils.throwReadableException("$message#${ NO_DATA_FOR_CONDITION_ERROR }")
                    }
        ...
        def possibleFilter = mappingFilter(data) { condition ->
            ...
            switch (Condition.getByTitle(conditionType.toLowerCase()))
            {
                ...
                case Condition.NOT_CONTAINS_INCLUDING_EMPTY:
                    return
                    buildFilterParameterFromCondition(Comparison.NOT_CONTAINS_INCLUDING_EMPTY)
                case Condition.NOT_CONTAINS:
                                                                 ...
                                                                 String id)
    {
        ...
        def possibleFilter = mappingFilter(data) { condition ->
            ...
            switch (Condition.getByTitle(conditionType.toLowerCase()))
            {
                ...
                case Condition.NOT_EQUAL:
                    return
                    buildFilterParameterFromCondition(Comparison.NOT_EQUAL) as FilterParameter
                case Condition.GREATER:
     ...
     * @return настройки группировки в удобном формате
     */
    private List<List<FilterParameter>> mappingDateTypeFilters(Source source,
        ...
        def possibleFilter = mappingFilter(data) { condition ->
            ...
            switch (Condition.getByTitle(conditionType.toLowerCase()))
            {
                ...
                case Condition.BETWEEN:
                    def dateSet =
                        condition
                            .data as Map<String, Object> // тут будет массив дат или одна из них
                    def start
                    ...
                    else
                    {
                        ...
                        else
                        {
                            minDate = DashboardUtils.getMinDate(
                                attribute.attrChains().code.join('.'),
     ...
     * @return настройки группировки в удобном формате
     */
    private List<List<FilterParameter>> mappingStateTypeFilters(String subjectUUID,
                                                                List<List> data,
                                                                Attribute attribute,
                                                                String title,
                                                                String id)
    {
        ...
        def possibleFilter = mappingFilter(data) { condition ->
            String conditionType = condition.type
            Closure buildFilterParameterFromCondition = { Comparison comparison,
                                                          Attribute attr,
                                                          value ->
                return new FilterParameter(
                    title: title,
            ...
            switch (Condition.getByTitle(conditionType.toLowerCase()))
            {
                ...
                case Condition.TITLE_CONTAINS:
                    return buildFilterParameterFromCondition(
                        Comparison.STATE_TITLE_CONTAINS,
                        attribute,
                        condition.data
                    )
                case Condition.TITLE_NOT_CONTAINS:
                    return buildFilterParameterFromCondition(
                        Comparison.STATE_TITLE_NOT_CONTAINS,
                        attribute,
                        condition.data
                    )
                case [Condition.EQUAL_SUBJECT_ATTRIBUTE, Condition.EQUAL_ATTR_CURRENT_OBJECT]:
                    if (!condition.data)
                    {
                        String message =
                            messageProvider
                                .getConstant(NO_DATA_FOR_CONDITION_ERROR, currentUserLocale)
                        return
                        utils.throwReadableException("$message#${ NO_DATA_FOR_CONDITION_ERROR }")
                    }
                    def code = condition.data.code
                    ...
                    if (subjectAttributeType != attribute.type)
                    {
                        ...
                        utils
                            .throwReadableException("$message#${ SUBJECT_TYPE_AND_ATTRIBUTE_TYPE_NOT_EQUAL_ERROR }")
                    }
                    return new FilterParameter(
                        value: value, title: title,
     ...
     * @return настройки группировки в удобном формате
     */
    private List<List<FilterParameter>> mappingTimerTypeFilters(Source source,
                                                                Boolean fromDD,
                                                                List<List> data,
                                                                Attribute attribute,
                                                                String title,
        ...
        def possibleFilter = mappingFilter(data) { condition ->
            ...
            switch (Condition.getByTitle(conditionType.toLowerCase()))
            {
                case Condition.STATUS_CONTAINS:
                    ...
                    return
                    buildFilterParameterFromCondition(Comparison.CONTAINS, temAttribute, value)
                ...
                case Condition.GREATER:
                    ...
                    temAttribute.addLast(
                        new Attribute(
                            title: 'значение счётчика',
                            code: 'elapsed',
                            type: 'long'
                        )
                    )
                    def durationInMs = getTimerDurationInMsByConditionValue(
                        condition.data,
                ...
                case Condition.LESS:
                    ...
                    return
                    buildFilterParameterFromCondition(Comparison.LESS, temAttribute, durationInMs)
                case Condition.EQUAL:
                    def temAttribute = attribute.deepClone()
                    temAttribute.addLast(
                        new Attribute(
                    ...
                    def durationInMsMinuteStart =
                        getTimerDurationInMsByConditionValue(condition.data)
                    def durationInMsMinuteEnd = getTimerDurationInMsByConditionValue(
                        condition.data,
                        fromDD ? TIMER_ROUND_TYPE.ROUND_CEIL_BY_SECONDS :
                            TIMER_ROUND_TYPE.ROUND_CEIL_BY_MINUTES
                    )
                    return buildFilterParameterFromCondition(
                        Comparison.BETWEEN,
                        temAttribute,
                        [durationInMsMinuteStart, durationInMsMinuteEnd]
                    )
                case Condition.NOT_EQUAL:
                    def temAttribute = attribute.deepClone()
                    temAttribute.addLast(
                ...
                case Condition.NOT_EMPTY:
                    def temAttribute = attribute.deepClone()
                    return
                    buildFilterParameterFromCondition(Comparison.NOT_NULL, temAttribute, null)
                default:
                    String message = messageProvider.getMessage(
                        NOT_SUPPORTED_CONDITION_TYPE_ERROR,
                        currentUserLocale,
    ...
    Long getTimerDurationInMsByConditionValue(ArrayList conditionValue,
                                              TIMER_ROUND_TYPE roundType =
                                                  TIMER_ROUND_TYPE.NO_ROUND)
    {
        def msInSecond = 1000
                                                      ...
                                                      Closure<FilterParameter> mapFilter)
    {
        return data.collect { andCondition ->
            andCondition.collect { orCondition -> mapFilter(orCondition)
            }
        }
    }
                       ...
                       PaginationSettings paginationSettings = null)
    {
        ...
        Boolean isSourceForEachRow =
            requestContent?.data?.sourceRowName?.findAll() && diagramType == DiagramType.TABLE
            ...
            return requisite.nodes.collectMany { node ->
                ...
                switch (nodeType.toLowerCase())
                {
                    case 'default':
                        ...
                        {
                            listIdsOfNormalAggregations = [0..(requestData.aggregations.size() - 1)]
                                .first()
                        }
                        Map<String, Object> filterMap =
                            getInfoAboutFilters(requisiteNode.dataKey, request)
                        String parameterSortingType =
                            diagramType == DiagramType.TABLE ? null : filterMap.parameterSortingType
                        ...
                        if (requestContent)
                        {
                            ...
                            String attrInCustoms =
                                getInnerCustomGroupNames(requestContent).find()?.attributeName
                        ...
                        Closure formatGroup = this.&formatGroupSet
                            .rcurry(newRequestData, listIdsOfNormalAggregations, diagramType)
                        def res = filtering?.withIndex()?.collectMany { filters, i ->
                            ...
                            {
                                def tempRes = [''] * (
                                    newRequestData.groups.size() + notAggregatedAttributes.size())
                                listIdsOfNormalAggregations.each { id -> tempRes.add(id, 0)
                                }
                                res = [tempRes].with(formatGroup)
                                               .with(formatAggregation)
                            }
                            def filtersTitle = filters.unique {
                                it.id
                            }.findResults {
                                if (it.title)
                                {
                            ...
                            filtersTitle = filtersTitle.unique()
                            def partial = (customInBreakTable || onlyFilled) && !res ? [:] :
                                [(filtersTitle): res]

                            partial = formatResult(
                                partial,
                                aggregationCnt + notAggregatedAttributes.size()
                            )
                            Boolean hasStateOrTimer = newRequestData?.groups?.any { value
                                ->
                                Attribute.getAttributeType(value?.attribute) in [
                                    AttributeType.STATE_TYPE, AttributeType.TIMER_TYPE]
                            } ||
                                                      newRequestData?.aggregations?.any {
                                                          it?.type in
                                                          Aggregation.NOT_APPLICABLE &&
                                                          Attribute
                                                              .getAttributeType(it?.attribute) in [
                            ...
                            if (hasStateOrTimer)
                            {
                                ...
                                partial = prepareRequestWithStates(
                                    partial,
                                    listIdsOfNormalAggregations,
                                    resWithPercentCnt
                                )
                            ...
                            filterListSize =
                                checkTableForSize(filterListSize, requestContent, diagramType)
                            return prepareResultListListForTop(
                                partial,
                                filterListSize,
                                top,
                                parameterFilters,
                        ...
                        Boolean parameterWithDateOrDtInterval =
                            parameterAttributeType in [*AttributeType.DATE_TYPES,
                                                       AttributeType.DT_INTERVAL_TYPE]
                    ...
                    case 'computation':
                        ...
                        def dataSet = calculator.variableNames.collectEntries {
                            ...
                            parameterSortingType = diagramType == DiagramType.TABLE ? '' :
                                filterMap.parameterSortingType
                            ...
                            List filters = filterListSize > 0 ? prepareFilters(
                                filterListSize,
                                diagramType,
                                requestContent,
                                parameterFilters,
                        ...
                        if (filterListSize == 0)
                        {
                            parameterSortingType = diagramType == DiagramType.TABLE ? null :
                                dataSet.values().head().groups.find()?.sortingType
                            return getNoFilterListDiagramData(
                                node,
                                request,
                                aggregationCnt,
                        ...
                        def variables = dataSet.collect { key, data ->
                            ...
                            return filtering.collect { filters ->
                                ...
                                Closure postProcess = this.&formatGroupSet.rcurry(
                                    newData as RequestData,
                                    listIdsOfNormalAggregations,
                                    diagramType
                                )
                                def res = dashboardQueryWrapperUtils.getData(
                                    newData as RequestData,
                                    top,
                                    currentUserLocale,
                                    user,
                                ...
                                {
                                    def tempRes = [''] * (
                                        newData.groups.size() + notAggregatedAttributes.size())
                                    listIdsOfNormalAggregations.each { id -> tempRes.add(id, 0)
                                    }
                                    res = [tempRes]
                                }
                        ...
                        Boolean hasStateOrTimer = groups?.any { value
                            ->
                            Attribute.getAttributeType(value?.attribute) in [
                                AttributeType.STATE_TYPE, AttributeType.TIMER_TYPE]
                        } ||
                                                  aggregations?.any {
                                                      it?.type ==
                                                      Aggregation.NOT_APPLICABLE &&
                                                      Attribute.getAttributeType(it?.attribute) in [
                                                          AttributeType.STATE_TYPE,
                                                          AttributeType.TIMER_TYPE]
                                                  }
     ...
     */
    List prepareFilters(Integer filterListSize,
                        DiagramType diagramType,
                        def requestContent,
                        List parameterFilters = [],
                        List breakdownFilters = [])
    {
        ...
        {
            if (diagramType == DiagramType.TABLE && (
                countOfCustomsInFirstSource > 1 || countOfCustomsInFullRequest > 1))
            {
                return breakdownFilters
                    ...
                    first + second
                } : parameterFilters*.inject { first, second -> first + second
                }
//фильтры могут быть от разных параметров, поэтому можно взять их уже в готовом виде
            }
            else
            {
     ...
     */
    List prepareResultListListForTop(List res,
    ...
    Map<String, Object> getInfoAboutFilters(String dataKey, DiagramRequest request)
    {
        ...
        String parameterSortingType =
            parameterInfo?.sortingType ?: request.data[dataKey].groups?.find()?.sortingType
        //если фильтры в разбивке, а упорядочиваем по параметру
    ...
    Integer getSortingDataIndex(def requestContent)
    {
        ...
        return requestContent.data.findIndexOf {
            it.dataKey == dataKeyForSorting
        }
    }
        ...
        return ids.collectMany { id ->
            variables.values().collect { el ->
                if (el)
                {
                    def value = id > 0
                        ? el.transpose()[0..id - 1] +
                          el.transpose()[(id + 1)..el.transpose().size() - 1]
                        : el.transpose().tail()
    ...
    {
        Boolean diagramTypeStacked = (diagramType == DiagramType.BAR_STACKED) || (
            diagramType == DiagramType.COLUMN_STACKED)
        Boolean isTypeMetaClass = request?.data?.any { key, value ->
            value.aggregations.any {
                it.type == Aggregation.PERCENT
            }
        }
     ...
     */
    Collection<Integer> getIndexesForTableWithNoParametersByAggregationType(DiagramRequest request,
                                                                            Aggregation aggregationFormat)
    {
        Collection<Integer> percentCntAggregationIndexes = []
                        ...
                        DiagramType diagramType)
    {
        ...
        def countNA = data.aggregations?.count {
            it?.type == Aggregation.NOT_APPLICABLE
        }
        ...
        {
            List notAggregated = data.aggregations.findAll {
                it.type == Aggregation.NOT_APPLICABLE
            }  //ищем агрегации n/a
            ...
            return list.collect { el
                ...
                def groups = []
                groups.addAll(el)
                //резервируем значения для групп
                def elAggregations = el[listIdsOfNormalAggregations]
                //резервируем значения для агрегаций
                elAggregations.each {
                    groups.remove(groups.indexOf(it))
                }
                //убираем в группах агрегации

                //обрабатываем группы
                def totalGroupValues = groups.withIndex().collect { group, i ->
                    return formatGroup(
                        requestGroups[i] as GroupParameter,
                ...
                elAggregations.eachWithIndex() { aggr, i
                    ->
                    totalGroupValues.add(listIdsOfNormalAggregations[i], aggr)
                }
                //возвращаем итог
     ...
     */
    private String formatGroup(GroupParameter parameter,
                               String fqnClass,
                               Source source,
                               def value,
                               DiagramType diagramType,
                               Boolean fromNA = false,
                               Boolean fromBreakdown = false)
    {
        ...
        switch (type)
        {
            case GroupType.OVERLAP:
                ...
                if (diagramType ==
                    DiagramType.TABLE ||
                    Attribute.getAttributeType(parameter.attribute) ==
                    AttributeType.META_CLASS_TYPE)
                {
                ...
                switch (Attribute.getAttributeType(parameter.attribute))
                {
                    ...
                    case AttributeType.STATE_TYPE:
                        ...
                        def (stateValue, stateCase) =
                        StateMarshaller.unmarshal(value, StateMarshaller.delimiter)
                        String totalFqn = (fqnClass.contains('$') || !stateCase) ? "${ fqnClass }" :
                            "${ fqnClass }\$${ stateCase }"
                        String userEq =
                            stateValue ? metainfo.getStateTitle(totalFqn, stateValue) : null
                    ...
                    case AttributeType.TIMER_TYPES:
                        ...
                        if (parameter.attribute.attrChains()?.last()?.timerValue ==
                            TimerValue.VALUE)
                        {
                    ...
                    case [AttributeType.CATALOG_ITEM_TYPE, AttributeType.CATALOG_ITEM_SET_TYPE]:
                        ...
                        {
                            def (tempValue, tempUuid) =
                            diagramType == DiagramType.TABLE ? [value, uuid] :
                                LinksAttributeMarshaller.unmarshal(value)
                            def code = utils.get(tempUuid).code
                            return
                            LinksAttributeMarshaller.marshalCatalog(tempValue, code, tempUuid)
                        }
                    default:
                        ...
                        value = (value == null || value.equals('')) ?
                            getNullValue(diagramType, fromBreakdown) : value
            ...
            case GroupType.SEVEN_DAYS:
                ...
                SimpleDateFormat specialDateFormatter = new SimpleDateFormat(
                    "dd.MM.yy",
                    russianLocale
                )
                def minDate
                ...
                {
                    parameter.attribute =
                        dashboardQueryWrapperUtils.updateRefAttributeCode(parameter.attribute)
                    minDate = DashboardUtils.getMinDate(
            ...
            default:
                String message =
                    messageProvider
                        .getMessage(NOT_SUPPORTED_GROUP_TYPE_ERROR, currentUserLocale, type: type)
    ...
    private String getTimerValue(Long value)
    {
        ...
        def minutes = TimeUnit.MILLISECONDS.toMinutes(value) -
                      TimeUnit.MINUTES.toMinutes(TimeUnit.MILLISECONDS.toHours(value))
        def seconds = TimeUnit.MILLISECONDS.toSeconds(value) -
    ...
    private String getCorrectIntervalType(def millis, GroupType type)
    {
        ...
        if (tempValueLessOne || !(tempValue instanceof Integer))
        {
            ...
            def roundIdx =
                fractionalPart.contains('E') //значение может быть в экспоненциальной записи
                    ? fractionalPart.dropWhile {
                    it != '-'
                }.toCharArray()[-1].toString() as Long
                    //тогда берем число после нее - ровно столько нулей стоит до числа
                    : fractionalPart.takeWhile {
                    it == '0'
                }.size() + 1
            //иначе идём по числу до тех пор, пока не пройдут все нули и берем + 1 значение
     ...
     */
    private List<List> formatSetWithDynamicValues(List<List> res,
                                                  String templateUUID,
                                                  ...
                                                  DiagramType diagramType)
    {
        if (templateUUID)
        {
            ...
            if (dynInGroups)
            {
                Integer realDynAttributeIndex = requestData.groups.findIndexOf {
                    it == dynInGroups
                } + aggregationsCount
            ...
            {
                Integer realDynAttributeIndex = requestData.aggregations.findIndexOf {
                    it == dynInAggregations
                }
                return res.collect { row ->
                    def dynValue = row[0]
                    row.remove(0)
            ...
            key ? list?.collect {
                if (aggregationCnt > 1)
                {
                    return it.size() > aggregationCnt
                        ? [it[0..aggregationCnt - 1] ?: DECIMAL_FORMAT.format(0), key,
                           it[aggregationCnt..-1]].flatten()
     ...
     */
    private List prepareRequestWithStates(List res,
                                          List listIdsOfNormalAggregations,
            ...
            return stateValues.collect { value ->
                ...
                res.each {
                    ...
                    if (tempResValue == value)
                    {
                        ...
                        resValue = [aggergationSet.collectMany {
                            it
                        }, *tempResValue]
                    }
                }
                List totalAggregation = aggregationCnt > 1
                    ...
                    : [!resWithPercentCnt ? DECIMAL_FORMAT.format(
                    resValue[0].sum {
                        it[0] as Double
                    }
                ) : resValue[0][0][0]]
                ...
                totalAggregation
                    .eachWithIndex() { aggr, i -> resValue.add(listIdsOfNormalAggregations[i], aggr)
                    }
                return resValue
            }
        }
        ...
        switch (transposeDataSet.size())
        {
            ...
            case 3:
                ...
                if (reverseGroups)
                {
                    ...
                    labelsForDiagram = getTotalLabelsForDiagram(
                        labelsForDiagram,
                        groupFormat,
                        format,
                        changeLabels,
                ...
                else
                {
                    ...
                    labels = getTotalLabelsForDiagram(
                        labels,
                        groupFormat,
                        format,
                        changeLabels,
            ...
            default:
                String message =
                    messageProvider.getConstant(INVALID_RESULT_DATA_SET_ERROR, currentUserLocale)
     ...
     */
    private void checkAggregationAndBreakdownListSize(Set groupResult,
                                                      Set breakdownResult,
                                                      Integer dataSetNum = 1)
    {
     ...
     */
    private def getTotalLabelsForDiagram(def labels,
                                         String groupFormat,
                                         String format,
                                         Boolean changeLabels,
                                         Boolean reverseLabels)
    ...
    private RoundDiagram mappingRoundDiagram(List list,
                                             Integer countTotals,
                                             Boolean isAggregationTypePercentCnt)
    {
        def resultDataSet = list.head() as List<List>
        def transposeDataSet = resultDataSet.transpose()
        switch (transposeDataSet.size())
                                             ...
                                             IUUIDIdentifiable user)
    {
        ...
        def transposeDataSet = resultDataSet.transpose()
        Integer aggregationCnt = getSpecificAggregationsList(requestContent).count {
            it.aggregation != Aggregation.NOT_APPLICABLE
        }
        ...
        if (sourceRowNames)
        {
            ...
            attributes.head().name =
                requestContent.data.head().indicators.head().attribute.title ?: ''
        }
        ...
        else
        {
            ...
            if (innerCustomGroupNames)
            {
                ...
                resultDataSet =
                    prepareResultDataSet(resultDataSet, attributeNames, innerCustomGroupNames)
                transposeDataSet = resultDataSet.transpose()
                if (hasBreakdown)
                {
                    customValuesInBreakdown = innerCustomGroupNames.findAll {
                        it.attributeName == attributeNames.last()
                    }.value
                }
            }
                                      ...
                                      List breakdownValues = [])
    {
        if (hasBreakdown)
        {
            def breakdownAttributeValue = attributes.find {
                it.type == ColumnType.BREAKDOWN
            }
            Collection<Column> parameterColumns = attributes.findResults { attrValue ->
                ...
                if (attrValue.type == ColumnType.INDICATOR)
                {
                    return new AggregationColumn(
                        ...
                        columns:
                            getBreakdownColumns(breakdownValues, breakdownAttributeValue, attrValue)
                    )
                }
            }
     ...
     */
    List<AggregationBreakdownColumn> getBreakdownColumns(List breakdownValues,
                                                         def breakdownAttributeValue,
                                                         def aggregationAttributeValue,
     ...
     */
    List prepareResultDataSet(List resultDataSet,
        ...
        resultDataSet.each { row ->
            ...
            row.each { value ->
                if (value in innerCustomGroupNames.value)
                {
                    ...
                    def attributeName = innerCustomGroupNames.find {
                        it.value == value
                    }.attributeName
    ...
    List getChildrenByParent(ResRow parent, List tree)
    {
        ...
        tree.each { child ->
            ...
            boolean correctParent =
                JsonOutput.toJson(child?.parent) == JsonOutput.toJson(tempParent)
            boolean unique = !children.any {
                it?.value == child?.value && it?.key == child?.key && it?.count == child?.count
            }
     ...
     */
    List<Map<String, Object>> getSpecificAggregationsList(Widget widgetSettings,
                                                          Boolean isCompute = null)
    {
        def tempWidgetSettings = mapper.convertValue(widgetSettings, widgetSettings.getClass())
        ...
        return tempWidgetSettings?.data?.collectMany { value ->
            if (!value.sourceForCompute && value?.indicators)
            {
                return value?.indicators?.findResults { indicator ->
                    ...
                    if (isCompute != null)
                    {
                        conditionTrue = !(isCompute ^ indicator?.attribute?.type == 'COMPUTED_ATTR')
//xor
                    }
                    ...
                    if (conditionTrue)
                    {
                        ...
                        String currentSourceName =
                            currentSource != mainSource ? value.source.value.label.trim() :
                                mainSourceValue.label.trim()

                        if ((fullIndicatorsList.count {
                            it?.attribute?.title == attribute?.title
                        } > 1) && !attribute?.title?.contains(currentSourceName))
                        {
    ...
    List prepareDataSet(List list, def requestContent, Boolean showNulls, Boolean hasBreakdown)
    {
        ...
        List usual = list[0]
        //на первом месте в списке  результатов, нужно внедрить на нужные места данные из результатов вычислений

        if (!(aggregations.size() ==
              compAggregations.size() &&
              compAggregations.size() == 1)) // если результат не состоит из одного вычисления
        {
            List indexesOfComputeInRequest = aggregations.findIndexValues {
                it.name in compAggregations.name
            }

            List indexesOfNotAggregatedInRequest = usualAggregations.findIndexValues {
                it.aggregation == Aggregation.NOT_APPLICABLE
            }
            ...
            if (aggregations.size() == compAggregations.size())
            {
                ...
                indexesOfComputeInRequest = indexesOfComputeInRequest[1..-1]
                //на первом месте уже стоит результат первого вычисления
            }

            list[1..-1].eachWithIndex { listRow, i
                ->
                //идем по другим спискам, где уже есть вычисления, результат хранится на первом месте, i покажет место, где хранится индекс подстановки числа из вычислений
                usual.each { row ->
                    def range = usualAggregationSize + i..-1
                    def tempRow = indexesOfNotAggregatedInRequest
                        ? row[*indexesOfNotAggregatedInRequest, range]
                        : row[range]
                    tempRow = tempRow.collect {
                        it ?: 0
                    }.sort()
        ...
        if (!showNulls)
        {
            ...
            if (hasBreakdown)
            {
                def removed = usual.findAll {
                    it[0..aggregationSize - 1].every {
                        !it || it == "0"
                    }
                }
                usual -= removed
                usual = usual.collectMany { value ->
                    [*removed.findAll {
                        it[aggregationSize..-2] == value[aggregationSize..-2]
                    }, value]
                }
            }
            else
            {
                usual -= usual.findAll {
                    it[0..aggregationSize - 1].any {
                        !it || it == "0"
                    } || it[aggregationSize..-1].every {
                        it == ""
                    }
                }
            }
    ...
    Integer countDistinct(def attributeValue, String classFqn, IUUIDIdentifiable user)
    {
        ...
        else
        {
                                   ...
                                   AttributeType.CATALOG_ITEM_SET_TYPE])
            {
                ...
                attrCodesList = attrCodesList.collect {
                    it == 'title' ? 'code' : it
                }
            }
            String attrCode = attrCodesList.collect {
                it.replace('metaClass', 'metaClassFqn')
            }.join('.')
            def s = selectClause
                                      ...
                                      IUUIDIdentifiable user)
    {
        ...
        Boolean valuesInBasicBreakdownExceedLimit = !customValuesInBreakdown &&
                                                    breakdownValues.size() >
        ...
        Collection<Column> columns =
            collectColumns(attributes, hasBreakdown, customValuesInBreakdown ?: breakdownValues)
        ...
        if (paginationSettings)
        {
            id = reverseRowCount ? rowCount - paginationSettings.firstElementIndex :
        ...
        Integer parameterIndex = aggregationCnt + notAggregatedAttributeSize
        //индекс, с которого в строке начинаются значения параметров

        //подготовка данных
        if (hasBreakdown)
        {
            Integer indexToFind = 2
            //берём до предпоследнего значения в строке, на последнем месте - разбивка
            def groups = tempMaps.groupBy {
                it[parameterIndex..-(indexToFind)]
            }//группируем данные по параметрам (их значениям)
            data = formatDataForTableWithBreakdown(
                groups, valuesInBasicBreakdownExceedLimit, breakdownValues,
                aggregationCnt, notAggregatedAttributeNames, parameterIndex, showRowNum
            )
            rowCount = data.size()
            if (tableTop && tableTop < rowCount)
            {
                PaginationSettings tempPaginationSettings = new PaginationSettings(
                    pageSize: tableTop,
                    firstElementIndex: 0
                )
            ...
            if (sourceRowNames)
            {
                Map<String, List> dataGroupedBySources = data.groupBy {
                    it['Источник']
                }
                Integer dataId = 1
        ...
        else
        {
            if (innerCustomGroupNames ||
                (sorting?.accessor &&
                 attributes.find {
                     it?.name?.trim() == sorting?.accessor?.trim()
                 }
        ...
        if (!sourceRowNames)
        {
            ...
            aggregationColumns = allAggregationAttributes.findResults { name ->
                columns.find {
                    Boolean match =
                        it.attribute.title == name && !takenAggregationColumns.contains(it)
                    if (match)
                    {
        ...
        Collection<Integer> percentAggregationIndexes =
            getIndexesForTableWithNoParametersByAggregationType(request, Aggregation.PERCENT)
        if (totalColumn)
        {
                            ...
                            else
                            {
                                totalCount = data.sum {
                                    it.entrySet().sum {
                                        ...
                                        {
                                            Collection potentialValuesForPercentCntAggregation =
                                                it.value.split(' ')
                                            String countInPercentCntAggregation =
                                                potentialValuesForPercentCntAggregation.head()
        ...
        {
            columns
                .add(0, new NumberColumn(header: "", accessor: "ID", footer: "", show: showRowNum))
        }
        ...
        boolean isDynamicParameter =
            parameterAttribute?.attribute?.code?.contains(AttributeType.TOTAL_VALUE_TYPE)
        Boolean customValuesInParameter = innerCustomGroupNames.any {
            it.attributeName == attributeNames[parameterIndex]
        }
        ...
        if (!customValuesInParameter && parameterAttribute)
        {
            limitParameter = !ignoreLimits?.parameter &&
                             isDynamicParameter
                ? dashboardQueryWrapperUtils.countDistinctTotalValue(
                source,
        ...
        boolean isDynamicBreakdown =
            breakdownAttribute?.attribute?.code?.contains(AttributeType.TOTAL_VALUE_TYPE)
        ...
        LimitExceeded limitsExceeded = new LimitExceeded(
     ...
     */
    private void formatIndicatorBreakdownValues(Collection<Map<String, Object>> data,
                                                RequestData requestData)
    {
     ...
     */
    private List<Map<String, Object>> formatDataForTableWithBreakdown(def groups,
                                                                      boolean valuesInBasicBreakdownExceedLimit,
                                                                      List breakdownValues,
                                                                      int aggregationCnt,
                                                                      List notAggregatedAttributeNames,
                                                                      int parameterIndex,
                                                                      boolean showRowNum)
    {
        int id = 0
        return groups.collect { parameters, group
            ...
            if (valuesInBasicBreakdownExceedLimit)
            {
                ...
                group = group.findAll {
                    it.last().values().head() in breakdownValues
                }
            }
            ...
            {
                aggregations =
                    updateAggregations(group, aggregationCnt, notAggregatedAttributeNames)
            }

            Integer attributesCntWithoutParameters = parameterIndex + 1
            //parameterIndex - количество агрегаций в запросе + 1 - разбивка, остальное параметры
            def tempAggregations = aggregations.collate(attributesCntWithoutParameters)
            def repeatableBreakdowns = tempAggregations.groupBy {
                it.last()
            }.findResults { k, v -> v.size() > 1 ? k : null
            }
            def listOfAggregations = repeatableBreakdowns.collect { breakdownValue ->
                return tempAggregations.findAll {
                    it.last() == breakdownValue
                }
            }
    ...
    {
        def attrValue = tableSorting?.accessor ? attributes.find {
            it?.name?.trim() == tableSorting.accessor.trim()
        } : null

        if (attrValue)
        {
            def valueType = attrValue?.aggregation !=
                            Aggregation.NOT_APPLICABLE && attrValue.type == ColumnType.INDICATOR
            ...
            switch (valueType)
            {
                case ColumnType.INDICATOR:
                    ...
                    return tempMaps.sort {
                        factor * it[attrValue.name]?.findResult {
                            it
                        }?.toDouble()
                    }
                case ColumnType.PARAMETER:
                    def total = tempMaps.sort {
                        it[attrValue.name]?.find()?.toLowerCase()
                    }
    ...
    Boolean checkForDateInAttribute(Map attributeValue, Boolean flag)
    {
        return (!(attributeValue.attribute.type in AttributeType.DATE_TYPES) ||
                (attributeValue.group.way == Way.SYSTEM &&
                 ...
                 attributeValue
                     .group
                     .format in ['dd.mm.YY hh:ii', 'dd.mm.YY hh', 'dd', 'dd.mm.YY', 'dd MM'])) ?
            flag : false
    }
     ...
     */
    List<Map> updateAggregations(List<Map> group,
                                 Integer aggregationCnt,
                                 List notAggregatedAttributeNames)
    {
        return group.collectMany { value ->
            return (0..aggregationCnt - 1).collect {
                ...
                {
                    aggregation[1] =
                        notAggregatedAttributeNames?.contains(aggregation[0]) ? "" : "0"
                }
     ...
     */
    List<Map> prepareRowsWithBreakdown(List<Map> rows,
                                       Integer aggregationCnt,
                                       List<String> attributeNames,
                                       List<String> notAggregatedAttributeNames,
                                       List<String> breakdownValues)
    {
        List listAttributes = []
        if (aggregationCnt > 0)
        {
        ...
        return rows.collect { row ->
            ...
            listAttributes.each { aggregationName ->
                List aggregations = row.findResults {
                    it[aggregationName]
                }
                //сами значения агргегации на удаление из списка
                List removeAggregations = row.findAll {
                    it[aggregationName]
                }
                List newAggregations = breakdownValues.collect { value ->
                    def num = aggregations.findResult {
                        it[value]
                    }
    ...
    ResRow createResRow(Map map, List<Map> row, List<String> attributeNames, int aggregationCnt)
    {
        ...
        totalRow.count =
            index == attributeNames.size() - 1 && aggregationCnt > 0 ? row[0..aggregationCnt - 1] :
                null
        return totalRow
    }
    ...
    List getFullRowsNew(List totalRows, ResRow parent, List tempTree)
    {
        ...
        return [parent] + children.collectMany {
            getFullRowsNew(totalRows, it, tempTree)
        }
    }
    ...
    private List<Map<String, Object>> getAttributeNamesAndValuesFromRequest(Widget widgetSettings)
    {
        ...
        def parameterAttributes = tempWidgetSettings.data.collectMany { value ->
            if (!value.sourceForCompute)
            {
                ...
                value.parameters.collect { parameter ->
                    if (fullParametersList.count {
                        it?.attribute?.title == parameter?.attribute?.title
                    } > 1)
                    {
                        if (currentSourceName)
                        {
                            parameter
                                ?.attribute
                                ?.title = "${ parameter?.attribute?.title } (${ currentSourceName })"
                        }
                    }
        ...
        Boolean isSourceForEachRow =
            widgetSettings.data.sourceRowName.findAll() && (widgetSettings.type as DiagramType) ==
            DiagramType.TABLE
            ...
            value.parameters.each { parameter ->
                if (parameter?.group?.way == Way.CUSTOM)
                {
                    parameter?.group?.data?.subGroups?.each { sub ->
                        attributeMaps << [attributeName: parameter?.group?.data?.name, value:
                            sub?.name]
                    }
                }
            }
            ...
            value.breakdown?.each { breakdown ->
                if (breakdown?.group?.way == Way.CUSTOM)
                {
                    breakdown?.group?.data?.subGroups?.each { sub ->
                        attributeMaps << [attributeName: breakdown?.group?.data?.name, value:
                            sub?.name]
                    }
                }
            }
    ...
    private ComboDiagram mappingComboDiagram(List list,
                                             ...
                                             Integer countTotals)
    {
        List transposeSets = list.collect {
            (it as List<List>)?.transpose() ?: []
        }

        Set labels = transposeSets.withIndex().collectMany { set, i ->
            if (customsInBreakdown[i])
            {
                return set[2]?.findAll() ?: []
                //значения параметра стоят на 3-м месте в результатах запроса, если группировка в разбивке кастомная
            }
            else
            {
        ...
        Set diagramLabels = transposeSets.withIndex().collectMany { set, i ->
            if (customsInBreakdown[i])
            {
                return set[1]?.findAll() ?: []
                //значения кастомной группировки из разбивки стоят на 2-м месте в результатах запроса
            }
            else
            {
        ...
        Integer globalMaxSize = transposeSets.collect {
            it?.size()
        }?.max()
        Closure getsSeries = { Set labelSet,
            ...
            switch (transposeData.size())
            {
                ...
                default:
                    String message =
                        messageProvider
                            .getConstant(INVALID_RESULT_DATA_SET_ERROR, currentUserLocale)
                    utils.throwReadableException("$message#${ INVALID_RESULT_DATA_SET_ERROR }")
            }
        }
    ...
    Set getLabelsInCorrectOrder(Set labels, String group, String format, Boolean reverse)
    {
        ...
        {
            case [GroupType.MINUTES, GroupType.HOURS]: return
                minutesAndHoursInCorrectOrder(labels, format, reverse)
            ...
            case [GroupType.WEEK, GroupType.YEAR]: return
                weeksInCorrectOrder(labels, format, reverse)
    ...
    Set minutesAndHoursInCorrectOrder(Set labels, String format, Boolean reverse)
    {
        ...
        return labels.sort {
            reverse ? -formatter.parse(it).getTime() : formatter.parse(it)
        }
    }
    ...
    Set daysInCorrectOrder(Set labels, String format, Boolean reverse)
    {
        ...
        {
            return labels.sort {
                Calendar.getInstance().setTime(formatter.parse(it))
            }
        }
        return labels.sort {
            reverse ? -formatter.parse(it).getTime() : formatter.parse(it)
    ...
    Set weeksInCorrectOrder(Set labels, String format, Boolean reverse)
    {
        ...
        return labels.sort {
            reverse ? -formatter.parse(it.replace('-я', '')).getTime() : formatter.parse(it)
        }
    }
    ...
    Set quartersInCorrectOrder(Set labels, String format, Boolean reverse)
    {
        ...
        return labels.sort {
            reverse ? -formatter.parse(it.replace(' кв-л ', '')).getTime() : formatter.parse(it)
        }
    }
     ...
     */
    private def isCustomGroupFromBreakdown(def requestContent,
    ...
    NewParameter mappingDynamicAttributeCustomGroup(Attribute dynamicAttribute)
    {
        if (dynamicAttribute?.property == AttributeType.TOTAL_VALUE_TYPE)
        {
            ...
            def subGroupData = new SubGroupData(
                data: [title: 'Шаблон атрибута', uuid: uuidForTemplate],
                type: 'CONTAINS'
            )
            def subGroup = new SubGroup(data: [[subGroupData]], name: '')
            def customGroup = new CustomGroup(
                name: groupName,
                subGroups: [subGroup],
                type: groupType,
                id: ''
     ...
     */
    private FilterList getFilterList(NewParameter parameter,
                                     String subjectUUID,
                                     String place,
                                     Source source,
                                     Integer offsetUTCMinutes,
     ...
     * @return TOP Х данных
     */
    List getTop(List currentRes,
                Integer top,
                List parameterFilters = [],
                List breakdownFilters = [],
                Boolean fromNoOrTwoFiltersList = false,
                def parameterWithDate = null,
                String parameterOrderWithDates = '',
                String aggregationOrderWithDates = '')
    {
        Integer paramIndex = 1
        //индекс, на котором расположены значения параметра (первого параметра для таблицы)
        Integer aggregationIndex = 0
        //индекс, на котором расположены значения показателя (первого показателя для таблицы)
        ...
        def tempResult = currentRes.groupBy {
            it[paramIndex]
        }.collect { k, v -> [k, v]
        }
        ...
        if (parameterWithDate && !aggregationOrderWithDates)
        {
            ...
            tempResult = getLabelsInCorrectOrder(
                labels,
                parameterWithDate.type as String,
                parameterWithDate.format,
                parameterOrderWithDates != 'ASC'
            ).toList()
            if (labelsHaveEmptyValue)
            {
        ...
        return tempResult.collectMany { value ->
            currentRes.findAll {
                it[paramIndex] == value
            }
        }
                                            ...
                                            PaginationSettings paginationSettings = null)
    {
        ...
        switch (nodeType.toLowerCase())
        {
            case 'default':
                ...
                def parameter = requestData.groups.find()
                String parameterSortingType =
                    isDiagramTypeTable && !top ? '' : parameter?.sortingType
                ...
                Closure formatGroup = this.&formatGroupSet
                    .rcurry(requestData, listIdsOfNormalAggregations, diagramType)
                def res = dashboardQueryWrapperUtils.getData(
                    requestData,
                    top,
                    currentUserLocale,
                ...
                Boolean hasStateOrTimer = requestData?.groups?.any { value
                    ->
                    Attribute.getAttributeType(value?.attribute) in [AttributeType.STATE_TYPE,
                                                                     AttributeType.TIMER_TYPE]
                } ||
                                          requestData?.aggregations?.any {
                                              it?.type ==
                                              Aggregation.NOT_APPLICABLE &&
                                              Attribute.getAttributeType(it?.attribute) in [
                                                  AttributeType.STATE_TYPE,
                ...
                if (hasStateOrTimer)
                {
                    ...
                    total = prepareRequestWithStates(
                        total,
                        listIdsOfNormalAggregations,
                        resWithPercentCnt
                    )
                }
                return totalPrepareForNoFiltersResult(
                    top,
                    isDiagramTypeTable,
                    tableHasBreakdown,
                    total,
            ...
            case 'computation':
                ...
                if (!checkGroupTypes(dataSet.values()))
                {
                    String message =
                        messageProvider
                            .getConstant(WRONG_GROUP_TYPES_IN_CALCULATION_ERROR, currentUserLocale)
                    utils
                        .throwReadableException("$message#${ WRONG_GROUP_TYPES_IN_CALCULATION_ERROR }")
                }
                ...
                def variables = dataSet.collectEntries { key, data ->
                    Closure postProcess =
                        this.&formatGroupSet
                            .rcurry(data as RequestData, listIdsOfNormalAggregations, diagramType)
                    Collection result = dashboardQueryWrapperUtils.getData(
                        data as RequestData,
                        top,
                        currentUserLocale,
                        user,
                    ...
                    if (result.size() == 0 ||
                        result.first() in Collection && result.first().size() == 0)
                    {
                        result[0] = [0]
                    }
                ...
                Boolean hasState = dataSet.values().head().groups?.any { value
                    ->
                    Attribute.getAttributeType(value?.attribute) == AttributeType.STATE_TYPE
                } ||
                                   dataSet.values().head().aggregations?.any {
                                       it?.type ==
                                       Aggregation.NOT_APPLICABLE &&
                                       Attribute.getAttributeType(it?.attribute) ==
                                       AttributeType.STATE_TYPE
                                   }
                String aggregationSortingType =
                    dataSet.values().head().aggregations.find()?.sortingType
                def parameter = dataSet.values().head().groups.find()
     ...
     */
    private List totalPrepareForNoFiltersResult(Integer top,
                                                Boolean isDiagramTypeTable,
                                                Boolean tableHasBreakdown,
                                                List total,
                                                def parameter,
                                                Boolean parameterWithDate,
                                                ...
                                                List percentCntAggregationIndexes)
    {
        if (top)
        {
            ...
            {
                def tempPaginationSettings = new PaginationSettings(
                    pageSize: top,
                    firstElementIndex: 0
                )
        ...
        Boolean nessecaryToSort = !parameterWithDateOrDtInterval &&
                                  (aggregationSortingType || parameterSortingType) &&
                                  (diagramType in
                                   DiagramType.SortableTypes || (top && isDiagramTypeTable))
        if (nessecaryToSort)
        {
            total = sortResList(
                total,
